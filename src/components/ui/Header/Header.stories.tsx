...
const meta: Meta<typeof Header> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          "Page chrome that frames every IFMIS screen. `Header` owns the",
          "gradient background and layout; you plug content into the `brand`",
          "and `actions` slots. It's a **compound component** — compose only",
          "the parts you need.",
          "",
          "### How to use it",
          ...
          "  actions={",
          "    <HeaderActions>",
          "      <AccessibilityMenu open={a11yOpen} onClick={toggleA11y} />",
          "      <LanguageToggle value={lang} onChange={setLang} />",
          "      <FontSizeControl value={size} onChange={setSize} />",
          "      <NotificationButton count={5} onClick={openInbox} />",
          "      <ProfileMenu user={user} onSettings={...} onLogout={signOut} />",
          "    </HeaderActions>",
          "  }",
          "/>",
          "```",
          "",
          "### Slots & parts",
          "- `brand` — left side, usually `<HeaderBrand title subtitle />`.",
          "- `actions` — right side; wrap items in `<HeaderActions>` for the",
          "  canonical spacing.",
          "- `backgroundImage` — optional; the brand gradient renders without",
          "  it. The library ships no default image — pass your own URL.",
          "",
          "Every action part is **controlled** (you own `value`/`open`/`count`",
          "and the handlers). `ProfileMenu` takes a `ProfileUser` object and",
          "falls back to initials when there's no avatar.",
        ].join("\n"),
      },
    },
  },
